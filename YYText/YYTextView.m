...
@interface YYTextView () <UIScrollViewDelegate, UIAlertViewDelegate, YYTextDebugTarget, YYTextKeyboardObserver> {
    ...
    NSMutableDictionary<NSNumber*, UIColor*>* colorDic;
...
- (void)_trackDidLongPress {
    ...
    if (_state.showingHighlight) {
        ...
        if ([self.delegate respondsToSelector:@selector(textViewDidLongPress:)]) {
            dealLongPressAction = YES;
            [self.delegate textViewDidLongPress:self];
            [self _endTouchTracking];
        }
    }
...
- (void)setBackgroundColor:(UIColor *)color forState:(UIControlState)state {
    if (colorDic == nil) {
        colorDic = [@{@(state): color} mutableCopy];
    } else {
        [colorDic setObject:color forKey:@(state)];
    }
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    _state.touchMoved = NO;
    _state.hasTapAction = [self.delegate respondsToSelector:@selector(textViewDidTap:)];
    _state.hasLongPressAction = [self.delegate respondsToSelector:@selector(textViewDidLongPress:)];
    _state.longPressed = NO;
    ...
    if (((!self.selectable && !_highlight) || _state.ignoreTouchBegan) && !_state.hasTapAction && !_state.hasLongPressAction) {
        _state.swallowTouch = NO;
        _state.trackingTouch = NO;
    }
    if (_highlight == nil) {
        self.backgroundColor = colorDic[@(UIControlStateHighlighted)];
    }
    if (_state.trackingTouch) {
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    _trackingPoint = point;
    if (_highlight == nil) {
        self.backgroundColor = colorDic[@(UIControlStateNormal)];
    }
    if (!_state.touchMoved) {
        _state.touchMoved = [self _getMoveDirection];
    }
    if (_state.trackingTouch) {
        ...
        }  else if (!_state.touchMoved && _state.hasTapAction) {
            if (!_state.longPressed) {
                [self.delegate textViewDidTap:self];
            }
        } else {
